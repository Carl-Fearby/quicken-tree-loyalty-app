...
export function AdviceLinks({links}: {links: {label: string; href: string}[]}) {
  return <div className="resource-callout"><p className="eyebrow">KEEP EXPLORING</p><div className="resource-grid">{links.map(link => <article key={link.href}><h2>{link.label}</h2><Link href={link.href}>Explore this page <span>↗</span></Link></article>)}</div></div>;
}
