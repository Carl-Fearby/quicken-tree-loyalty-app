...
export const metadata: Metadata = {
  ...
  alternates: {languages: {'en-GB': siteUrl ? `${siteUrl}/` : '/'}},
...
export default function RootLayout({children}: Readonly<{children: React.ReactNode}>) {
  const schema = { '@context': 'https://schema.org', '@graph': [
    ...
    { '@type': 'SoftwareApplication', name: 'Pace', applicationCategory: 'BusinessApplication', operatingSystem: 'Web', description, featureList: ['Restaurant booking diary', 'Table management', 'Menu maintenance', 'Customer loyalty points', 'Kitchen closing times', 'Companion guest app'], offers: [{ '@type': 'Offer', price: '59', priceCurrency: 'GBP', description: 'Pace Essentials starting price per venue per month' }], ...(siteUrl ? { url: siteUrl, publisher: { '@id': `${siteUrl}/#organization` } } : {}) },
  ] };
