export type Content = {
 ...
 menu:{serviceMessages:Record<string,string>;menuServicePeriods:{id:string;label:string;start:string;end:string;categories:string[];days?:number[]}[];dietaryTagNames:Record<string,string>;dietaryTags:Record<string,string[]>;dishImages?:Record<string,string>;allergenTagNames?:Record<string,string>;allergenTagStyles?:Record<string,{color:string;icon:string}>;allergenTags?:Record<string,string[]>;outOfStockItems:string[];menuItems:Record<string,{title:string;items:string[][]}[]>;categories:{label:string;source:string;service:string;sections?:number[]}[]};
