(() => {
  ...
  function getDialog() {
    ...
    dialog.innerHTML =
      '<form method="dialog"><h2>Database management</h2><p>Enter the password to continue.</p><input id="database-gate-input" type="password" placeholder="Password" required><p id="database-gate-error"></p><div class="database-gate-actions"><button type="button" id="database-gate-cancel">Cancel</button><button value="unlock">Unlock</button></div></form>';
    document.body.append(dialog);
    return dialog;
  }
  function openDatabase() {
    window.dispatchEvent(new Event('quicken-tree-open-database-management'));
  }
