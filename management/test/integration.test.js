...
test(
  'PostgreSQL deletion, dependency protection and rollback',
  ...
  async () => {
    ...
    const parent = `management_test_parent_${suffix}`,
      child = `management_test_child_${suffix}`;
    ...
    try {
      await assert.rejects(
        sql.begin(async (tx) => {
          ...
          await tx.unsafe(
            `CREATE TABLE public."${child}" (id integer PRIMARY KEY,parent_id integer REFERENCES public."${parent}" ON DELETE CASCADE)`,
          );
